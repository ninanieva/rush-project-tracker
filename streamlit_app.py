...
# Added "Approval Status" right before Remarks for sheet alignment
PROJECT_COLS = [
...
DEFAULT_SETTINGS = {
    "doc_types": ["System Design","Feature Spec","Integration Doc","Release Notes","CRF"],
...
with tab1:
    ...
    with st.form("create_form", clear_on_submit=True):
        ...
        with c2:
            # Conditional field logic based on chosen external Document Type selection
            if doc_type == "CRF":
    ...
    if projects:
        ...
        with c2:
            ...
            fig = px.bar(x=pt.values, y=pt.index, orientation='h', title="Total Price by Tribe",
                         color_discrete_sequence=[RUSH_TEAL])
            fig.update_layout(paper_bgcolor=RUSH_WHITE, plot_bgcolor=RUSH_WHITE, font=dict(color=RUSH_DARK), xaxis_title="PHP", yaxis_title="")
            st.plotly_chart(fig, use_container_width=True)
        ...
        ml = [{"Project": p.get("Project Name"), "Merchant": p.get("Merchant"),
               ...
               "Price": f"PHP {float(p.get('Project Price',0) or 0):,.2f}",
