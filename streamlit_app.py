...
/* Compact Icon-Free Export Button Link Style */
.rush-download-link {{
    display: block;
    text-align: center;
...
with tab2:
    ...
    # ── Interceptor Filter Reset State Engine ────────────────────────
    ...
    # Enhanced Omni-Search Element (Queries Project Name, Merchant, or Control Number)
    ...
    # ── Operational Control Grid Row (Smaller, Emoji-Free Button Link) ──
    ...
    # ── High-Performance Native Matrix Display Table View ───────────
    if not df_display.empty:
        # Re-map absolute workspace headers to isolate unique target parameters
        ...
        # Display static structured matrix grid configuration with internal header sorting disabled
        st.dataframe(
            df_table_view,
            use_container_width=True,
            hide_index=True,
            column_config={
                "Control #": st.column_config.TextColumn("Control #", sortable=False),
                "Project Name": st.column_config.TextColumn("Project Name", sortable=False),
                "Merchant": st.column_config.TextColumn("Merchant", sortable=False),
                "Pipeline Status": st.column_config.TextColumn("Pipeline Status", sortable=False),
                "Doc Type": st.column_config.TextColumn("Doc Type", sortable=False),
                "Project Price": st.column_config.NumberColumn("Project Price", format="PHP %,.2f", sortable=False),
                "PO Status": st.column_config.TextColumn("PO Status", sortable=False),
                "Project Status": st.column_config.TextColumn("Project Status", sortable=False)
            }
        )
        
        # ── Dedicated Inline Row Modification Access Blocks ─────────────
        st.markdown("---")
        st.markdown("##### Record Modifications Control Hub")
        edit_selector_names = [f"{p.get('Control Number')} - {p.get('Project Name')}" for p in filtered]
        selected_project_opt = st.selectbox("Select Record Row to Modify / Remove", edit_selector_names)
        
        if selected_project_opt:
            selected_ctrl_id = selected_project_opt.split(" - ")[0]
            p_idx, p = next((i, records) for i, records in enumerate(filtered) if records.get("Control Number") == selected_ctrl_id)
            
            with st.expander(f"⚙️ Open Workspace Editor for Row ID: {p.get('Control Number')}", expanded=True):
                ...
                edit_price = st.number_input("Price (PHP)", value=float(p.get('Project Price', 0) or 0), min_value=0.0, key=f"e_pr_{p_idx}")
                ...
                with c_sav:
                    if st.button("Update Record Entry", key=f"save_edit_{p_idx}", use_container_width=True, type="primary"):
                        if not edit_name or edit_doc_type == "":
                            ...
                            st.error("Reason for Rejection and Rejector are mandatory fields.")
                        else:
                            ...
                            if update_project_row(p.get("ID"), updated_p):
                                st.session_state.toast_notification = "Project entries successfully updated!"
                                st.rerun()
                                
                with c_del:
                    if st.button("Delete Record Entry Permanently", key=f"del_{p_idx}", use_container_width=True, type="secondary"):
                        if delete_project_row(p.get("ID","")):
                            st.session_state.toast_notification = "Project dropped from spreadsheet array system."
                            st.rerun()
    else:
